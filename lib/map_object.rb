...
class MapObject
  ...
  def id()
    i = map.map_objects.find_index(self)
    i ? i : map.map_objects.length
  end
  ...
  def name=(val)
    ...
    # TODO: Search themes for name and set chamber purpose attributes if relevent
  end
  ...
  def add_connector(connector, connector_offset = 0, cursor: @cursor, direction: :right)
    ...
    debug "#{id_str}: Adding connector - cursor: #{tmp_cursor.to_s}"
    square(tmp_cursor.pos).add_connector(tmp_cursor.facing, connector)
  ...
  def has_incomplete_connectors?()
    ...
    if incomplete_connectors.length > 0 or incomplete_doors.length > 0
      debug "Incomplete for #{name}: #{incomplete_connectors.length} connectors, #{incomplete_doors.length} doors"
      return true
    end
    debug "No incomplete connectors or doors for #{name}"
    return false
  end
  ...
  def draw_starting_connector(cursor: @cursor)
    ...
    for i in 1..@width do
      ...
      next if square(tmp_cursor.pos).nil?
      if @map.square(tmp_cursor.map_pos_forward) and @map.square(tmp_cursor.map_pos_forward).edges[tmp_cursor.facing(:back)] == @starting_connector
        if starting_connector.kind_of? Door
          square(tmp_cursor.pos).add_door(tmp_cursor.facing, starting_connector)
        else
          square(tmp_cursor.pos).add_connector(tmp_cursor.facing, starting_connector)
        end
      end
    end
  end
  ...
  def draw_forward(distance, cursor: @cursor, width: @width)
    for i in 1..distance do
      ...
      debug "#{id_str}: OK to draw forward at #{cursor.map_pos_forward}, #{cursor.facing}"
      cursor.forward!()
  ...
  def draw_width(cursor: @cursor, width: @width)
    ...
    set_square(tmp_cursor.pos, MapObjectSquare.new(self, {tmp_cursor.left => :wall}))
    for i in 1...width do
      tmp_cursor.shift!(:right)
      set_square(tmp_cursor.pos, MapObjectSquare.new(self))
    end
    square(tmp_cursor.pos).add_wall(tmp_cursor.right)
  end
  ...
  def add_wall_width(cursor: @cursor, width: @width, direction: :right)
    ...
    begin
      tmp_cursor = cursor.copy()
      square(tmp_cursor.pos).add_wall(tmp_cursor.facing)
      for i in 1...width do
        tmp_cursor.shift!(direction)
        square(tmp_cursor.pos).add_wall(tmp_cursor.facing)
      end
  ...
  def remove_wall_width(cursor: @cursor, width: @width, direction: :right)
    tmp_cursor = cursor.copy()
    square(tmp_cursor.pos).remove_wall(tmp_cursor.facing)
    for i in 1...width do
      tmp_cursor.shift!(direction)
      square(tmp_cursor.pos).remove_wall(tmp_cursor.facing)
    end
